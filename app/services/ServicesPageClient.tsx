...
const SERVICE_IMAGES = [
  ...
  img("DSC09380_aonznj"),
  img("DSC01456_ygk3gp"),
  img("DSC01346-Enhanced-NR_lf6bof"),
];

type DbService = { number: string; title: string; subtitle: string; description: string; tags: string; ctaText: string };
type DbStep = { number: string; title: string; description: string };

type ServiceCardData = { num: string; label: string; title: string; desc: string; tags: string[]; src: string; cta: string };

function dbServicesToCards(services: DbService[]): ServiceCardData[] {
...
const FALLBACK_SERVICES: ServiceCardData[] = [
  { num: "01", label: "Photography", title: "Commercial & Product Stills", desc: "Full-frame photography for automotive campaigns, product launches, editorial portraits, and brand identity.", tags: ["Automotive", "Product", "Editorial", "E-Commerce"], src: img("DSC01488_qx6a1n"), cta: "Book a Shoot" },
  { num: "02", label: "Cinematography", title: "Brand & Narrative Films", desc: "Cinematic brand stories and product films — shot on cinema glass, graded in DaVinci Resolve, delivered in 4K.", tags: ["Brand Films", "Product Cinematics", "Weddings", "Documentary"], src: img("DSC09380_aonznj"), cta: "Commission a Film" },
  { num: "03", label: "Social Content", title: "Reels & Platform Video", desc: "Short-form content optimised for Instagram, YouTube, and broadcast — reels, demos, testimonials.", tags: ["Instagram Reels", "YouTube", "Product Demo", "Events"], src: img("DSC01456_ygk3gp"), cta: "Start a Project" },
  { num: "04", label: "Aerial & BTS", title: "Drone & Behind-the-Scenes", desc: "Licensed drone cinematography for aerial establishing shots, event coverage, and authentic BTS content.", tags: ["Licensed Drone Ops", "Aerial", "Event Coverage", "BTS"], src: img("DSC01346-Enhanced-NR_lf6bof"), cta: "Enquire Now" },
];
...
export default function ServicesPageClient({ dbServices, dbSteps }: { dbServices?: DbService[]; dbSteps?: DbStep[] }) {
  const SERVICES = dbServices?.length ? dbServicesToCards(dbServices) : FALLBACK_SERVICES;
  const steps = dbSteps?.length
    ? dbSteps.map(s => ({ n: s.number, t: s.title, b: s.description }))
    : FALLBACK_STEPS;

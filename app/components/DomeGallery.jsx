...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      animEl.className = 'enlarge-closing';
      ...
      if (origImg) { const img = origImg.cloneNode(); img.style.cssText = 'width:100%;height:100%;object-fit:cover;'; animEl.appendChild(img); }
      overlay.remove();
      rootRef.current.appendChild(animEl);
      void animEl.getBoundingClientRect();
      requestAnimationFrame(() => {
        animEl.style.left = origRel.left + 'px'; animEl.style.top = origRel.top + 'px';
        animEl.style.width = origRel.width + 'px'; animEl.style.height = origRel.height + 'px';
  ...
  const openItemFromElement = useCallback(el => {
    ...
    if (openedImageWidth || openedImageHeight) {
      const onFirstEnd = ev => {
        ...
        overlay.style.width = tmpW; overlay.style.height = tmpH;

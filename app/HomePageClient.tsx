...
type Settings = {
  heroTagline?: string; heroCta?: string; heroCtaLink?: string;
  founderName?: string; founderTitle?: string; founderBio1?: string; founderBio2?: string;
  founderAvatarUrl?: string; availabilityText?: string;
  ctaHeading?: string; ctaBtn1Text?: string; ctaBtn1Link?: string; ctaBtn2Text?: string; ctaBtn2Link?: string;
};

export default function HomePageClient({ settings }: { settings?: Settings }) {
  const s = settings ?? {};
  const heroTagline   = s.heroTagline   ?? "Vika Films";
  const heroCta       = s.heroCta       ?? "Let's Create ↗";
  const heroCtaLink   = s.heroCtaLink   ?? "/contact";
  const founderName   = s.founderName   ?? "Vivek Kamble";
  const founderTitle  = s.founderTitle  ?? "Founder & Cinematographer, VIKA FILMS";
  const founderBio1   = s.founderBio1   ?? "I create cinematic content for luxury hospitality brands, premium products, healthcare companies, and businesses looking to elevate their visual identity.";
  const founderBio2   = s.founderBio2   ?? "My work focuses on combining storytelling, aesthetics, and commercial strategy to create films that leave a lasting impression.";
  const avatarUrl     = s.founderAvatarUrl ?? "/vivek-kamble.png";
  const availability  = s.availabilityText ?? "Available across India & International";
  const ctaBtn1Text   = s.ctaBtn1Text   ?? "Book A Shoot";
  const ctaBtn1Link   = s.ctaBtn1Link   ?? "/contact";
  const ctaBtn2Text   = s.ctaBtn2Text   ?? "View Portfolio";
  const ctaBtn2Link   = s.ctaBtn2Link   ?? "/portfolio";
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* CTA pill */}
          <Link href={heroCtaLink} className="hero-cta">
            {heroCta}
          </Link>
        </div>
      </section>
